...
def fetch_weather_alerts():
    ...
    try:
        ...
        exclusion_terms = ["Small Craft Advisory", "Gale Warning", "Gale Watch", "Open Water", "Child Abduction Emergency", "AMBER Alert"]
        ...
        for entry in root.findall("atom:entry", ns):
            ...
            area = entry.find("atom:category[@term='area']", ns)
            area = area.attrib.get("label") if area is not None else "Unknown Area"
...
def save_to_spreadsheet(alerts):
    ...
    # Create a DataFrame
    df = pd.DataFrame(alerts, columns=["State", "County", "Title", "Area", "Updated", "WHAT", "WHERE", "WHEN", "IMPACTS", "ADDITIONAL DETAILS", "INSTRUCTIONS"])
...
def send_email_with_attachment(alert_message, num_alerts, attachment_path):
    ...
    try:
        subject = f"Daily Severe Weather Alerts - {num_alerts} Alert(s) Found"
        msg = MIMEMultipart()
...
def daily_weather_alert():
    ...
    alert_message = f"Attached is the daily weather alert report with {num_alerts} alert(s)."
